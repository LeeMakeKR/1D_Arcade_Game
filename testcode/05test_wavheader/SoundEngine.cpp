...
// 사운드 데이터. 
// 스케치 전체에서 이 파일만 SoundData.h를 include해야 한다.
//
// 소리를 몇 개만 쓰는 게임 코드라면 SoundData.h 대신 개별 헤더를 골라 include하면
// 나머지는 플래시를 차지하지 않는다. 예) #include "sounds/laser.h" -> CLIP_laser 사용
...
// 동시 재생 슬롯 하나. 플래시의 어느 PCM 배열을 몇 번째 샘플까지 재생했는지만 들고 있으면 된다.
struct Voice {
...
// 활성 슬롯 수를 다시 센다.
// 이 값은 LCD 표시용이기만 한 게 아니라, audioTask가 "믹싱을 계속 돌릴지 큐에서 잠들지"를
// 판단하는 기준이다. 그래서 슬롯을 건드린 직후에는 반드시 갱신해야 한다.
...
static void startVoice(uint8_t id) {
  if (id >= SND_COUNT) return;

  const SoundClip &clip = SOUND_CLIPS[id];
  if (clip.pcm == nullptr || clip.len == 0) return;
...
// 여러 스위치를 거의 동시에 눌러도 요청이 이 루프에서 한 틱 안에 모두 처리되므로 함께 겹쳐 난다.
//
// core 0을 쓰는 이유: Arduino의 setup()/loop()는 core 1에 고정된 태스크에서 돌기 때문에,
// 오디오를 core 0에 두면 스위치/LED/LCD 처리와 물리적으로 다른 코어에서 진짜 동시에 돈다.
static void audioTask(void*) {
  uint8_t id;
  for (;;) {
    TickType_t wait = (activeVoices == 0) ? portMAX_DELAY : 0;
    while (xQueueReceive(reqQueue, &id, wait) == pdTRUE) {
      startVoice(id);
      wait = 0;
    }
...
void soundEngineBegin(int bclkPin, int lrcPin, int doutPin) {
  ...
  // (파일마다 레이트가 다른 wav를 그대로 재생하던 방식과 달리 믹싱이 단순해지는 이유)
  i2s_config_t config = {
    ...
    .dma_buf_count = 6,    // LED show()나 LCD 갱신이 잠깐 CPU를 잡아도 언더런되지 않을 정도의 여유
    .dma_buf_len = 128,
    .use_apll = false,
...
void soundPlay(uint8_t id) {
  xQueueSend(reqQueue, &id, 0);  // 큐가 가득 차면 버린다(호출한 쪽을 절대 블로킹하지 않는 것이 우선)
}
...
const char* soundName(uint8_t id) {
  return (id < SND_COUNT) ? SOUND_CLIPS[id].name : "";
}
...
uint32_t soundTotalBytes() {
  uint32_t total = 0;
  for (uint8_t i = 0; i < SND_COUNT; i++) total += SOUND_CLIPS[i].len;
  return total;  // 8bit PCM이라 샘플 수가 곧 바이트 수
}
